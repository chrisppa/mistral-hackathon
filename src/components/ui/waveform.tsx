  ...
  useEffect(() => {
    ...
    const renderWaveform = () => {
      ...
      for (let i = 0; i < barCount; i++) {
        ...
        // find the transcript message for the current speaker. We dont have the timestamp here, but we do have the index and total duration
        const percentage = i / barCount;
        // find the approx timestamp of this bar
        const timestamp = percentage * duration;
        // find the first transcript message that has a timestamp greater than or equal to the timestamp
        const transcriptMessage = transcript.find(
          (message) => message.timeInCallSecs >= timestamp
        );
        console.log(transcriptMessage?.message);

        // console.log({ transcriptMessage });


        // ctx.fillStyle = computedBarColor
        ctx.fillStyle = transcriptMessage?.role === 'agent' ? agentColor : userColor
